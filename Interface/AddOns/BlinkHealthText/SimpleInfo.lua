...
function S:UpdateUnitFrame()
	...
	--脱离战斗
		--print("脱离战斗");
		if(heal<maxheal or (powertype=="MANA" and power<maxpower)) then
		--血、蓝未满
			--print("血、蓝未满");
			if (UnitExists("target") and (not UnitIsFriend("player","target"))) then
			--有目标,且非友方目标
				--print("有目标,且非友方目标");
				self.frame["player"]:SetAlpha(0.5);
				self.frame["target"]:SetAlpha(0.5);
				self.frame["player"]:Show();
				self.frame["target"]:Show();
			else
			--无目标，或者友方目标
...
--	end
	...
	if (UnitExists("target")) then
		...
		if(IsAddOnLoaded('RealMobHealth')) then
			local hpRMH, maxhpRMH = RealMobHealth.GetUnitHealth("target", true)

			if hpRMH and maxhpRMH then
				heal, maxheal = hpRMH, maxhpRMH
			end
		end
		...
		if (UnitExists("targettarget")) then
			...
			if(IsAddOnLoaded('RealMobHealth')) then
				local hpRMH, maxhpRMH = RealMobHealth.GetUnitHealth("targettarget", true)
...
function BlinkHealth_SlashHandler(msg)
	...
	local class1 = select(2, UnitClass("player"));
	...
	elseif (cmdtype == "hiton") then
		if (class1 == "ROGUE" or class1 == "DRUID") then	
...
function S:BlinkHealth_Options_Init()
	if not BlinkHealth_Switch then 
		BlinkHealth_Switch = {player = 1, target = 1, combo = 1, cast = 1};
	end
	if not BlinkHealth_Player_Switch then 
		BlinkHealth_Player_Switch = {name = 1,heal = 1,power = 1,healP = 1,powerP = 1};
	end
end