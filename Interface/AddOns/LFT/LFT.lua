...
LFTRoleCheck:SetScript("OnHide", function()
    if LFT.isLeader then
        if LFT.findingMore then
            ...
            lfprint(LFT.L['A member of your group has not confirmed his role.'])
...
LFTRoleCheck:SetScript("OnUpdate", function()
    ...
    if gt >= st then
        ...
        if LFT.isLeader then
            lfprint(LFT.L['A member of your group does not have the '] .. COLOR_HUNTER .. '[LFT] ' .. COLOR_WHITE ..
            LFT.L['addon. Looking for more is disabled. (Type '] .. COLOR_HUNTER .. '/lft advertise ' .. COLOR_WHITE .. ' to send them a link)')
...
LFTWhoCounter:SetScript("OnShow", function()
    ...
    lfprint(LFT.L['Checking people online with the addon (5secs)...'])
end)
...
LFTGroupReadyFrameCloser:SetScript("OnUpdate", function()
    ...
    if gt >= st2 then
        ...
        lfprint(LFT.L['A member of your group has not accepted the invitation. You are rejoining the queue.'])
        if LFT.isLeader then
    ...
    if event then
        ...
        if event == 'CHAT_MSG_ADDON' and arg1 == LFT_ADDON_CHANNEL then
            ...
            if string.sub(arg2, 1, 10) == 'roleCheck:' then
                ...
                lfprint(LFT.L['A role check has been initiated. Your group will be queued when all members have selected a role.'])
                ...
                --    end
                --else
                _G['LFTRoleCheckQForText']:SetText(COLOR_WHITE .. LFT.L["Queued for "] .. COLOR_YELLOW .. LFT.dungeonNameFromCode(mCode))
            ...
            if string.sub(arg2, 1, 11) == 'acceptRole:' then
                ...
                if arg4 == me then
                    lfprint(LFT.L['You have chosen: '] .. roleColor .. LFT.ucFirst(roleEx[2]))
                end
                ...
                if roleEx[2] == 'tank' then
                    ...
                    if LFT_ROLE == 'tank' then
                        if _G['LFTRoleCheck']:IsVisible() then
                            ...
                            if LFT.isLeader then
                                if arg4 ~= me then
                                    lfprint(LFT.classColors[LFT.playerClass(arg4)].c .. arg4 .. COLOR_WHITE .. LFT.L[' has chosen ']
                                            .. COLOR_TANK .. LFT.L['Tank'] .. COLOR_WHITE .. LFT.L[' but you already confirmed this role.'])
                                    lfprint(LFT.L['Queueing aborted.'])
                                ...
                                if LFT.LFMGroup.tank ~= '' and LFT.LFMGroup.tank ~= me then
                                    lfprint(COLOR_TANK .. LFT.L['Tank'] .. COLOR_WHITE .. LFT.L['role has already been filled by '] .. LFT.classColors[LFT.playerClass(LFT.LFMGroup.tank)].c .. LFT.LFMGroup.tank
                                            .. COLOR_WHITE .. LFT.L['. Please select a different role to rejoin the queue.'])
                                    return false
                                end
                            end
                        end
                    ...
                    if LFT_ROLE == 'healer' then
                        if _G['LFTRoleCheck']:IsVisible() then
                            ...
                            if LFT.isLeader then
                                if arg4 ~= me then
                                    lfprint(LFT.classColors[LFT.playerClass(arg4)].c .. arg4 .. COLOR_WHITE .. LFT.L[' has chosen ']
                                            .. COLOR_HEALER .. LFT.L['Healer'] .. COLOR_WHITE .. LFT.L[' but you already confirmed this role.'])
                                ...
                                if LFT.LFMGroup.healer ~= '' then
                                    lfprint(COLOR_HEALER .. LFT.L['Healer'] .. COLOR_WHITE .. LFT.L['role has already been filled by '] .. LFT.classColors[LFT.playerClass(LFT.LFMGroup.healer)].c .. LFT.LFMGroup.healer
                                            .. COLOR_WHITE .. LFT.L['. Please select a different role to rejoin the queue.'])
                                    return false
                                end
                            end
                                ...
                                if LFT.isLeader then
                                    if arg4 ~= me then
                                        if LFT.LFMGroup.damage1 ~= '' and LFT.LFMGroup.damage2 ~= '' and LFT.LFMGroup.damage3 ~= '' then
                                            lfprint(LFT.classColors[LFT.playerClass(arg4)].c .. arg4 .. COLOR_WHITE .. LFT.L[' has chosen '] .. COLOR_DAMAGE .. LFT.L['Damage'] .. COLOR_WHITE
                                                    .. LFT.L[' but the group already has '] .. COLOR_DAMAGE .. '3' .. COLOR_WHITE .. ' confirmed ' .. COLOR_DAMAGE .. LFT.L['Damage'] .. COLOR_WHITE .. ' members.')
                                            lfprint('Queueing aborted.')
                                            leaveQueue('4 dps')
                                            return false
                                        end
                                    ...
                                    if LFT.LFMGroup.damage1 ~= '' and LFT.LFMGroup.damage2 ~= '' and LFT.LFMGroup.damage3 ~= '' then
                                        lfprint(COLOR_DAMAGE .. LFT.L['Damage'] .. COLOR_WHITE .. LFT.L['role has already been filled by '] .. COLOR_DAMAGE .. '3' .. COLOR_WHITE .. LFT.L[' members. Please select a different role to rejoin the queue.'])
                                        return false
                                    end
                                end
                            end
                ...
                if arg4 ~= me then
                    lfprint(LFT.classColors[LFT.playerClass(arg4)].c .. arg4 .. COLOR_WHITE .. LFT.L[' has chosen: '] .. roleColor .. LFT.ucFirst(roleEx[2]))
                end
                LFT.checkLFMgroup()
            end
        ...
        if event == 'CHAT_MSG_CHANNEL' and arg8 == LFT.channelIndex and string.find(arg1, 'lft_group_formed', 1, true) then
            ...
            if LFT_CONFIG['spamChat'] then
                lfnotice(LFT.dungeonNameFromCode(code) .. LFT.L[' group just formed. (type "/lft spam" to disable this message)'])
            end
            if me == 'Er' then
                local totalGroups = 0
                ...
                lfprint(groupsFormedThisSession .. LFT.L[' this session, '] .. totalGroups .. LFT.L[' total recorded.'])
            end
            if not time then
                return false
            end
        ...
        if event == 'CHAT_MSG_CHANNEL' and string.find(arg1, '[LFT]', 1, true) and arg8 == LFT.channelIndex and arg2 ~= me and --for lfg
            ...
            LFT.readyStatusReset()
            _G['LFTGroupReadyObjectivesCompleted']:SetText('0/' .. LFT.tableSize(LFT.bosses[LFT.groupFullCode]) .. LFT.L[' Bosses Defeated'])
            _G['LFTGroupReady']:Show()
            LFTGroupReadyFrameCloser:Show()
            _G['LFTRoleCheck']:Hide()
...
function lfprint(a)
    if a == nil then
        DEFAULT_CHAT_FRAME:AddMessage(COLOR_HUNTER .. '[LFT]|cff0070de:' .. time() .. LFT.L['|cffffffff attempt to print a nil value.'])
        return false
    end
...
LFT:SetScript("OnEvent", function()
    if event then
        ...
        if event == "PARTY_MEMBERS_CHANGED" then
                ...
                if LFT.findingMore --then
                    ...
                    if LFT.group[LFT.LFMDungeonCode].healer ~= '' and LFT.group[LFT.LFMDungeonCode].healer ~= me then
                        ...
                        if not stillInParty then
                            ...
                            LFT.LFMGroup.healer = ''
                            lfprint(leftName .. ' (' .. COLOR_HEALER .. LFT.L['Healer'] .. COLOR_WHITE .. LFT.L[') has been removed from the queue group.'])
                        end
                    end
                    --
                    if LFT.group[LFT.LFMDungeonCode].damage1 ~= '' and LFT.group[LFT.LFMDungeonCode].damage1 ~= me then
                        ...
                        if not stillInParty then
                            ...
                            lfprint(leftName .. ' (' .. COLOR_DAMAGE .. LFT.L['Damage'] .. COLOR_WHITE .. LFT.L[') has been removed from the queue group.'])
                        end
                    end
                    --
                    ...
                    if LFT.group[LFT.LFMDungeonCode].damage3 ~= '' and LFT.group[LFT.LFMDungeonCode].damage3 ~= me then
                        ...
                        if not stillInParty then
                            ...
                            lfprint(leftName .. ' (' .. COLOR_DAMAGE .. LFT.L['Damage'] .. COLOR_WHITE .. LFT.L[') has been remove from the queue group.'])
                        end
                    end
                end
            end
            ...
            local awesomeButton = _G['LFTGroupReadyAwesome']
            awesomeButton:SetText(LFT.L['Waiting Players ('] .. LFT.groupSizeMax - GetNumPartyMembers() - 1 .. ')')
            awesomeButton:Disable()

            if GetNumPartyMembers() == LFT.groupSizeMax - 1 then
                awesomeButton:SetText(LFT.L['Let\'s do this!'])
...
function LFT.init()
    ...
    if LFT_TYPE == TYPE_ELITE_QUESTS then
        _G['LFTMainDungeonsText']:SetText(LFT.L['Elite Quests'])
        _G['LFTBrowseDungeonsText']:SetText(LFT.L['Elite Quests'])
    else
        _G['LFTMainDungeonsText']:SetText(LFT.L['Dungeons'])
        _G['LFTBrowseDungeonsText']:SetText(LFT.L['Dungeons'])
    end
...
LFTQueue:SetScript("OnUpdate", function()
    ...
    if gt >= st and LFT.findingGroup then
        ...
        _G['LFTGroupStatusTimeInQueue']:SetText(LFT.L['Time in Queue: '] .. SecondsToTime(time() - LFT.queueStartTime))
        if LFT.averageWaitTime == 0 then
            _G['LFTGroupStatusAverageWaitTime']:SetText(LFT.L['Average Wait Time: Unavailable'])
        else
            _G['LFTGroupStatusAverageWaitTime']:SetText(LFT.L['Average Wait Time: '] .. SecondsToTimeAbbrev(LFT.averageWaitTime))
        end
...
function LFT.fillAvailableDungeons(queueAfter, dont_scroll)
    ...
    if queueAfter then
        ...
        if LFT.dungeons[dungeonName].canQueue then
            ...
            lfprint(LFT.L['A member of your group does not meet the suggested minimum level requirement for |cff69ccf0'] .. dungeonName)
        end
    end
...
function LFT.weInQueue(code)
    ...
    lfprint(LFT.L['Your group is in the queue for |cff69ccf0'] .. dungeonName)
...
function LFT.LFTBrowse_Update()
                ...
                if data.queued and (LFT.findingMore or LFT.findingGroup) then
                    ...
                    if not LFT.inGroup and queues < 5 then
                        ...
                        if LFT.dungeonsSpamDisplay[data.code].tank == 0 and string.find(LFT_ROLE, 'tank', 1, true) then
                            ...
                            _G["BrowseFrame_" .. data.code .. "_JoinAs"]:SetText(LFT.L['Join as Tank'])
                        ...
                        elseif LFT.dungeonsSpamDisplay[data.code].healer == 0 and string.find(LFT_ROLE, 'healer', 1, true) then
                            ...
                            _G["BrowseFrame_" .. data.code .. "_JoinAs"]:SetText(LFT.L['Join as Healer'])
                            _G["BrowseFrame_" .. data.code .. "_JoinAs"]:Show()
                        elseif LFT.dungeonsSpamDisplay[data.code].damage < 3 and string.find(LFT_ROLE, 'damage', 1, true) then
                            ...
                            _G["BrowseFrame_" .. data.code .. "_JoinAs"]:SetText(LFT.L['Join as Damage'])
                            _G["BrowseFrame_" .. data.code .. "_JoinAs"]:Show()
                        end
                    end
                end
    ...
    if dungeonIndex > 0 then
        ...
        _G['LFTBrowseBrowseText']:SetText(LFT.L['Browse'].. '(' .. dungeonIndex .. ')')
        _G['LFTMainBrowseText']:SetText(LFT.L['Browse'].. '(' .. dungeonIndex .. ')')
    else
        _G['LFTBrowseNoPeople']:Show()
        _G['LFTBrowseBrowseText']:SetText(LFT.L['Browse'])
        _G['LFTMainBrowseText']:SetText(LFT.L['Browse'])
    end
...
function sayNotReady()
    if not LFT.dungeonNameFromCode(LFT.groupFullCode) then
        return
    end
    if LFT.inGroup and GetNumPartyMembers() + 1 == LFT.groupSizeMax then
        _G['LFTGroupReady']:Hide()
...
function LFT_ShowMinimap()
    ...
    if LFT.findingGroup or LFT.findingMore then
        ...
        GameTooltip:AddLine(LFT.L['Looking For Turtles - LFT'], 1, 1, 1)
        GameTooltip:AddLine(LFT.L['Left-click to open LFT.'])
        GameTooltip:AddLine(LFT.L['Drag to move.'])
        GameTooltip:AddLine(LFT.L['You are not queued for any dungeons.'])
        if LFT.peopleLookingForGroupsDisplay == 0 then
            GameTooltip:AddLine(LFT.L['No players are looking for groups at the moment.'])
        elseif LFT.peopleLookingForGroupsDisplay == 1 then
            GameTooltip:AddLine(LFT.peopleLookingForGroupsDisplay .. LFT.L[' player is looking for groups at the moment.'])
        else
            GameTooltip:AddLine(LFT.peopleLookingForGroupsDisplay .. LFT.L[' players are looking for groups at the moment.'])
        end
        GameTooltip:Show()
    end
end
...
function findGroup()
    ...
    local roleText = ''
    if string.find(LFT_ROLE, 'tank', 1, true) then
        roleText = roleText .. COLOR_TANK .. LFT.L['Tank']
    end
    if string.find(LFT_ROLE, 'healer', 1, true) then
        local orText = ''
        if roleText ~= '' then
        ...
        roleText = roleText .. orText .. COLOR_HEALER .. LFT.L['Healer']
    end
    if string.find(LFT_ROLE, 'damage', 1, true) then
        local orText = ''
        if roleText ~= '' then
        ...
        roleText = roleText .. orText .. COLOR_DAMAGE .. LFT.L['Damage']
    end
    ...
    lfprint(LFT.L['You are in the queue for |cff69ccf0'] .. dungeonsText ..
            COLOR_WHITE .. LFT.L[' as: '] .. roleText)
...
function leaveQueue(callData)
    ...
    if LFT.findingGroup or LFT.findingMore then
        if LFT.inGroup then
            ...
            lfprint(LFT.L['Your group has left the queue for |cff69ccf0'] .. dungeonsText .. COLOR_WHITE .. '.')
        else
            if callData ~= 'from join queue' then
                lfprint(LFT.L['You have left the queue for |cff69ccf0'] .. dungeonsText .. COLOR_WHITE .. '.')
            end
        end
...
SlashCmdList["LFT"] = function(cmd)
    if cmd then
        ...
        if string.sub(cmd, 1, 3) == 'who' then
            ...
            if LFT.channelIndex == 0 then
                lfprint(LFT.L['LFT.channelIndex = 0, please try again in 10 seconds'])
                return false
            end
            LFTWhoCounter:Show()
        ...
        if string.sub(cmd, 1, 17) == 'resetformedgroups' then
            LFT.resetFormedGroups()
            lfprint(LFT.L['Formed groups history reset.'])
        end
        if string.sub(cmd, 1, 12) == 'formedgroups' then
            lfprint(LFT.L['Listing formed groups history'])
            local totalGroups = 0
            ...
            if totalGroups == 0 then
                lfprint(LFT.L['There are no recorded formed groups.'])
            else
                lfprint(LFT.L['There are '] .. totalGroups .. LFT.L[' recorded formed groups.'])
            end
        end
...
function LFT.sendAdvertisement(chan)
    SendChatMessage(LFT.L['I am using LFT - Looking For Turtles - LFG Addon for Turtle WoW v'] .. addonVer, chan, DEFAULT_CHAT_FRAME.editBox.languageID)
        ...
        for i, d in DefaultChannels do
            if d == LFT.channel then
                if getglobal("ChatFrame" .. windowIndex) then
                    ...
                    lfprint(LFT.L['LFT channel removed from window '] .. windowIndex .. ' LFT')
                    lfnotice(LFT.L['Please do not type in the LFT channel or add it to your chat frames.'])
                end
            end
        end
    end
