...
local BlizzUI = { "ActionBar", "BonusActionButton", "MainMenu", "ShapeshiftButton", "MultiBar", "KeyRingButton", "PlayerFrame", "TargetFrame", "PartyMemberFrame", "ChatFrame", "TargetofTargetFrame", "WorldFrame", "ActionButton", "CharacterMicroButton", "SpellbookMicroButton", "TalentMicroButton", "QuestLogMicroButton", "SocialsMicroButton", "LFGMicroButton", "HelpMicroButton", "CharacterBag", "PetFrame",  "MinimapCluster", "MinimapBackdrop", "UIParent", "WorldFrame", "Minimap", "BuffButton", "BuffFrame", "TimeManagerClockButton", "ChatFrameMenuButton", "ExhaustionTick" };
local UserUIProtected = { "MinimapButtonFrameDragButton", "TitanPanel", "AutoBarButton", "FuBarFrame", "RicoMinimap_CoordinatesFrame", "MinimapZoom", "MinimapButtonFrame", "Xparky", "BasicMiniMap", "CT_RASetsFrame", "OneBagFrame", "ChatBarFrame", "diminfo_Bag", "diminfo_dura", "diminfo_Friends", "diminfo_Guild", "diminfo_pos", "diminfo_System", "diminfo_Time", "pfQuestMapTracker", "pfUITracking", "SimpleMiniMap", "PINGY_MSG_Pinger", "PINGY_MSG_Leader", "PINGY_MSG_Class"};
local MinimapIcons = { "Note", "JQuest", "Naut_", "MinimapIcon", "GatherMatePin", "WestPointer", "Chinchilla_", "SmartMinimapZoom","QuestieNote", "smm" };
local ParentStop = { "WorldFrame", "Minimap", "MinimapBackdrop", "UIParent", "MinimapCluster", "CECBMiniMapButtonFrame", "CT_RASetsFrame" };

MBF_Default_Ignore = { "MetamapButton", "MinimapButtonFrameDragButton", "MinimapButtonFrame" };

for i = 1, 150 do
	table.insert(MBF_Default_Ignore, "pfMiniMapPin"..i)
end

for k = 1, 30 do
	table.insert(MBF_Default_Ignore, "FWGMinimapPOI"..k)
end

MBF_Default_Include = { "DPSMate_MiniMap","EVTButtonFrame","KTM_MiniMapButtonFrame"};

MBF_FrameLocation = {"CENTER", "CENTER" , 0, 0};
MBF_Vars = { 5, 3, .8, false, 1, false, false, "Nothing", true, true, false, false, 1, { 0, 0, 0}, true, "Alpha", false, false, false, false, false, false, true, false };
...
--小地图图标渐隐
function FadeMiniMapButton()
	local timeout = 0.3 --渐隐速度
	if MouseIsOver(Minimap, 15, -15, -15, 15) then
		this.activeTo = GetTime() + timeout
		this:SetAlpha(1)
	elseif this.activeTo then
		if this.activeTo < GetTime() and this:GetAlpha() > 0 then
...
function MBF_OnUpdate(elapsed)
	...
	if (paused == false) then
		...
		if ((activeScanning) and (start_time > 1) and (scanned)) then
			MBF_Scan();
			start_time = 0;
		end
	end
...
function fixNils()
	...
	for k,v in ipairs(BlizzButtons) do
		if ((v ~= "MiniMapBattlefieldFrame") and (v ~= "MiniMapMeetingStoneFrame")) then
...
isValidAdd = function(frame, autoGather)
	...
	if (frameName ~= nil) then
		...
		elseif (frameName == "MiniMapMailFrame") then
			if ((disabledMail == false) or ((grabBlizzButtons == false) and (isInTable(MBF_Include, frameName) == false)) or ((grabBlizzButtons == true) and (isInTable(MBF_Ignore, frameName) == true))) then
				result = false;
			end
		else
			result = false;
...
displayDisabledMail = function()
	...
	local result = true;
	if ((disabledMail == false) or (MiniMapMailFrame:IsShown())) or ((grabBlizzButtons == true) and (isInTable(MBF_Ignore,"MiniMapMailFrame") == true)) or ((grabBlizzButtons == false) and (isInTable(MBF_Include,"MiniMapMailFrame") == false)) then
		result = false;
	end
...
local sortChildren = function()
	...
	for i, button in ipairs({MinimapButtonFrame:GetChildren()}) do
		...
		if ((button:IsVisible()) or (mbfHidden)) then
			...
			if ((sortOrder == "BlizzFirst") or (sortOrder == "BlizzLast")) then
...
function updatePositions()
	...
	for i, buttonName in ipairs(sortChildren()) do
		...
		if (buttonName ~= "MinimapButtonFrameDragButton") then
...
function snapMinimap()  
	...
	MBFMiniButtonFrame:SetPoint("CENTER", Minimap, "CENTER", x/(addonScale/scale), y/(addonScale/scale));
		...
		MBFMiniButtonFrame:SetPoint("CENTER", Minimap, "BOTTOMLEFT", cX/(addonScale/scale), cY/(addonScale/scale));
