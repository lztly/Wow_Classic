	...
	if bagID > 0 then
		...
		if(link) then
			...
			if itemType == '弹药' then
				...
				return SUCC_bagOptions.colors.bag[subType], subType == '背包'
			end
		end
	end
	...
	elseif not button:GetParent().colorLocked then
		...
		if c or SUCC_bagOptions.colors.override then
			...
			if link then
				...
				if n ~= nil and string.find(n, '荣誉奖章') then
...
local function Essentials(frame)
	...
	frame.title:SetText(frame.title.t ~= '' and frame.title.t ~= nil and frame.title.t or '背包')
	if frame.slotFrame then
		...
		frame.toggleButton:SetScript('OnEnter', function()
			...
			GameTooltip:AddLine('左键: 打开背包', 1, 1, 1)
			GameTooltip:AddLine('右键: 打开钥匙袋' , 0.3, 0.8, 1)
			GameTooltip:Show()
		end)
		...
		if Clean_Up then
			...
			frame.cuBag:SetScript('OnEnter', function()
				GameTooltip:SetOwner(this, 'ANCHOR_LEFT')
				GameTooltip:AddLine('左键: 整理', 1, 1, 1)
				GameTooltip:AddLine('右键: 倒序整理' , 0.3, 0.8, 1)
				GameTooltip:Show()
			end)
...
local function OnEvent()
	...
	elseif event == 'ADDON_LOADED' and arg1 == 'SUCC-bag' then
		...
		-- configuration
		SLASH_SUCC_BAG1 = '/背包'
		print('|cFFF6A3EFSUCC-bag 加载.输入 /背包 - 进入配置')
	end
end
...
local function SetColumns()
	...
	if n == '背包' then FrameLayout(SUCC_bag, l) else FrameLayout(SUCC_bag.bank, l) end
end
...
local function CreateMenuFrame()
	...
	menu.header.t:SetText('SUCC-背包 控制面板')

	menu.bag = {}
	menu.bank = {}

	menu.bag.columns = slider('SBC_bagColumns', {'容器纵列', '4', '32'}, {4, 32}, {SUCC_bagOptions.layout.columns.bag, 1})
	menu.bag.columns:SetPoint('TOPLEFT', menu, 35, -45)
	menu.bag.columns:SetScript('OnValueChanged', SetColumns)

	menu.bank.columns = slider('SBC_bankColumns', {'银行纵列', '4', '32'}, {4, 32}, {SUCC_bagOptions.layout.columns.bank, 1}, menu.bag.columns)
	menu.bank.columns:SetScript('OnValueChanged', SetColumns)

	menu.spacing = slider('SBC_itemSpacing', {'物品间距', '0', '20'}, {0, 20}, {SUCC_bagOptions.layout.spacing, 1}, menu.bag.columns, 1)
	...
	menu.border = color('SBC_borderColor', '边框', SUCC_bagOptions.colors.border)
	...
	menu.backdrop  = color('SBC_backdropColor', '背景', SUCC_bagOptions.colors.backdrop, menu.border)
	...
	menu.item = color('SBC_itemColor', '物品边框', SUCC_bagOptions.colors.bag.Bag, menu.border, 1)
	...
	menu.quest = color('SBC_questColor', '任务物品', SUCC_bagOptions.colors.quest, menu.item)
	...
	menu.highlight = color('SBC_highlightColor', '高亮物品', SUCC_bagOptions.colors.highlight, menu.item, 1)
	...
	menu.bg = color('SBC_BGColor', '战场牌子', SUCC_bagOptions.colors.BG, menu.highlight)
	...
	menu.soul = color('SBC_soulColor', '灵魂袋', SUCC_bagOptions.colors.bag['Soul Bag'], menu.highlight, 1)
	...
	menu.herb = color('SBC_herbColor', '草药袋', SUCC_bagOptions.colors.bag['Herb Bag'], menu.soul)
	...
	menu.enchanting = color('SBC_enchantingColor', '附魔包', SUCC_bagOptions.colors.bag['Enchanting Bag'], menu.soul, 1)
	...
	menu.ammo = color('SBC_ammoColor', '弹药袋', SUCC_bagOptions.colors.ammo, menu.enchanting)
